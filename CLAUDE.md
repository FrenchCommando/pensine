...
- **Dispose discipline** — `TextEditingController`s created inside `showDialog` builders must be disposed via `.whenComplete(() => controller.dispose())` (or try/finally around `await showDialog`). `TextPainter`s in custom paint code must be disposed after their final `paint()`/`layout()`. Leak tracker (`test/flutter_test_config.dart`) fails CI on any undisposed `Disposable`.
