...
### Windows (plain zip — current channel)
- `release.yml`'s `windows-release` job builds `flutter build windows --release`, zips `build\windows\x64\runner\Release\*`, and attaches the zip to the GitHub Release tagged `build-<run_number>` alongside the APK. Unsigned — users click through SmartScreen on first launch. No store dependency.
- `ci.yml`'s `build-windows` job additionally uploads the zip as a workflow artifact on every push/PR so any green build is downloadable without cutting a release. See DEPLOYMENT.md for the user-facing install steps.
