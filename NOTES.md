...
- **Marble label on countdown boards (non-start items):** painted below center in the retro-LCD **VT323** face (`assets/fonts/VT323-Regular.ttf`, OFL-licensed, bundled in `pubspec.yaml`) — arcade-timer vibe. Reads `N (x/y)` where `y = durationSeconds` and `x` is `durationSeconds` before the step runs, the live remaining value while it's active, and `0` when the step is done. The remaining-seconds math lives in **one** pure helper (`lib/behavior/countdown_remaining.dart::countdownRemainingSeconds`) used by both the marble label (`board_screen.dart::_countdownRemainingForActive`) and the `_TimerOverlay` bottom chip — they previously ceiled and floored independently and drifted by one simultaneously on screen, which was very noticeable. **Consistency between the two displays is the priority.** Convention: **ceil of fractional remaining** — the `3..2..1..go` cadence. `dur=5` reads `5, 4, 3, 2, 1` each for ~1s, then the countdown `Timer` fires at `elapsed=5s` and auto-advances; the live display never renders `0`. The painter's `isDone → shown = 0` branch is what paints `0` on the next frame on the just-completed marble — so the transition is `1` (active, final second) → `0` (done) at advance. This is why the "set to zero when the marble becomes done" rule matters: the active-step helper naturally skips to the next undone step, but the completed marble keeps painting until the next frame, and without the `isDone` branch it would keep showing the last live value. Rebuilds are driven by the `_overlayTick` ValueNotifier — physics ticker may be idle while the active marble is static, so MarbleBoard is wrapped in a `ValueListenableBuilder<int>` for countdown boards to force one rebuild per second. Timer boards and checklists just show `N` in the default font — they have no duration per item.
...
- **Native-desktop integration tests are CI-only.** Any desktop target (Windows / macOS / Linux) builds a binary that shares the platform app-support directory and shared_preferences backing store with an installed Pensine — so local runs pollute the user's real workspaces with fixture data. Web and mobile targets are sandboxed (browser storage, emulator/simulator FS) and aren't affected. Two-layer guard: (a) `tool/run_windows_integration.sh` checks `CI=true` before invoking `flutter drive` on the Windows job — instant rejection without paying the ~30s build cost. CI sets `CI=true` automatically. (b) `requireCIForNativeDesktop()` in `integration_test/test_helpers.dart` fires inside the test main() as defense-in-depth if someone bypasses the script with raw `flutter drive`. On web + mobile the helper short-circuits (`kIsWeb` / non-desktop OS check), which also dodges `dart:io`'s `Platform.isWindows` throwing `UnsupportedError` on web (a runtime trap that hung the chrome job before the short-circuit landed). Override locally by setting `CI=true` in the shell — accepts the data pollution.
...
### Linux (AppImage + .deb)
- `release.yml`'s `linux-release` job builds `flutter build linux --release`, then produces two artifacts from the same bundle: a `.deb` (Debian/Ubuntu native install with `.desktop` entry + `.pensine` MIME registration) and an AppImage (single file, runs on any glibc 2.31+ distro). Both attached to the GitHub Release tagged `build-<run_number>` alongside the APK / .dmg / Windows artifacts.
- Packaging logic lives in `linux/packaging/` so the workflow stays thin: `build_deb.sh` stages the install tree under `/usr/lib/pensine/` + `/usr/bin/pensine` shim + desktop/icon/MIME files, writes `DEBIAN/control`, calls `dpkg-deb --build`. `build_appimage.sh` builds an `AppDir`, downloads `linuxdeploy` + `linuxdeploy-plugin-gtk` into a tmp dir (nothing checked in), `--appimage-extract`s linuxdeploy because GHA disables FUSE, runs the GTK plugin to bundle GDK/GIO/schemas + transitive .so deps.
- **File association** declared once in `linux/packaging/pensine-mime.xml` (`application/x-pensine` for `*.pensine`), referenced from `linux/packaging/pensine.desktop` (`MimeType=application/x-pensine;`), shared by both packages. The .deb's `postinst`/`postrm` refresh `update-mime-database` + `update-desktop-database` + `gtk-update-icon-cache` so the association is live without re-login. Incoming-file handoff mirrors the Windows pattern: `linux/runner/my_application.cc::HandleIncomingPensineFile` scans argv in `local_command_line` before the Flutter engine starts, copies the first `.pensine` file's bytes (capped at 10 MB) to `${g_get_tmp_dir()}/pensine_incoming.pensine`, and `pending_import_native.dart` picks it up on its cold-launch poll. `GApplication` is left `NON_UNIQUE` (Flutter default), so each double-click spawns a fresh process — the observer-on-resumed branch isn't strictly needed, but `Platform.isLinux` is included in the list for symmetry with the other desktop platforms.
- Both packages are unsigned. .deb users see one apt "untrusted origin" warning; AppImage has no such warning.
...
- No "submit to store" step in the automation — the pipeline uploads to Play internal + TestFlight only. Push commits to main, run the Release workflow from the Actions UI. That's the whole automated flow. For iOS production, add the TestFlight build to a new App Store version in App Store Connect and click "Add for Review"; **"Automatically release this version" is toggled on**, so approved builds go live on the App Store (`https://apps.apple.com/app/pensine/id6762313502`, ID `6762313502`) without another click.
...
- **iOS (TestFlight):** TestFlight app notifies when Apple finishes processing (~10–30min typical, first build after a plist change can be longer). External testers join via public link `https://testflight.apple.com/join/KDHvbWKH` (also linked from README + landing page); internal team members are added by Apple ID in App Store Connect → TestFlight → Internal Testing. To promote to the live App Store listing (`https://apps.apple.com/app/pensine/id6762313502`), add the build to a new App Store version and click **Add for Review**; auto-release-after-approval is enabled, so no second click is needed once Apple approves.
...
- `.github/workflows/integration.yml` — runs on push/PR to main; six parallel jobs (chrome / android / ios / macos / windows / linux) each run `flutter drive` against `integration_test/smoke_test.dart` on their respective platform; the three native-desktop jobs also run `pending_import_test.dart` to guard the argv/URL → temp file → Dart polling handoff. Complements `ci.yml` (unit/widget tests on `flutter_tester`) by exercising real platform channels: actual `shared_preferences`, web `dart:html` bindings, Android/iOS native lifecycle, and desktop file-association handoff. Chrome uses `browser-actions/setup-chrome@v2` + chromedriver; Android uses `reactivecircus/android-emulator-runner@v2` (KVM) with the same pixel_7 profile as `artifacts.yml`; iOS uses `macos-latest` + `tool/boot_ios_simulator.sh` wrapped in `nick-fields/retry@v3` (same flake mitigation as artifacts.yml); Linux uses `ubuntu-latest` + Xvfb for a virtual display. Wall time ~10-15 min dominated by iOS Xcode build.
...
- `.github/workflows/release.yml` — manual trigger only (`workflow_dispatch`); `android-release` uploads to Play internal + publishes signed APK as a GitHub Release tagged `build-<run_number>`, `ios-release` uploads to TestFlight, `macos-mas-release` uploads to Mac App Store, `macos-dmg-release` appends a notarized .dmg, `windows-release` appends zip + Inno Setup installer, `linux-release` appends AppImage + .deb — all attached to the same `build-<run_number>` tag (see DEPLOYMENT.md)
...
### Not yet set up
- `.pensine` file association is wired on every desktop + mobile target: iOS (`SceneDelegate`), Android (`ACTION_VIEW` + `ACTION_SEND`), web PWA (`file_handlers`), Windows (Inno Setup registry + `runner/utils.cpp`), macOS (`CFBundleDocumentTypes` + `AppDelegate.application(_:open:)`), Linux (`.desktop` MimeType + `linux/runner/my_application.cc::HandleIncomingPensineFile`).
