...
- **Linux AppImage + .deb reuse the macOS 512×512 icon.** `linuxdeploy --icon-file` only accepts freedesktop canonical sizes (8, 16, ..., 480, 512), and our `assets/app_icon.png` is 1024×1024. `linux/packaging/build_{appimage,deb}.sh` point at `macos/Runner/Assets.xcassets/AppIcon.appiconset/app_icon_512.png` — same byte stream, already generated by `flutter_launcher_icons`, no imagemagick dance at build time. If the macOS asset catalog is ever restructured, the Linux release will fail and this bullet is the breadcrumb.
...
- **Post-dialog assertions must scope the finder to the home Scaffold.** After a dialog closes and the test inspects the page behind it, do `find.descendant(of: find.byType(Scaffold), matching: find.text(x))` instead of a bare `find.text(x)`. iOS simulator's keyboard-dismiss + route-pop animation can outrun `settle()`'s 5s budget, leaving the dialog's `TextField` mounted with its typed contents; an unscoped finder then matches both the new page content and the live `EditableText` and the `findsOneWidget` assertion fails with "found 2 widgets." `AlertDialog` lives in a route overlay (not inside Scaffold), so the descendant scope is a timing-independent way to ignore still-animating dialogs. Caught this in `smoke_test.dart`'s "Create a new board" test — the fix was scoping the final `find.text` assertion to `find.byType(Scaffold)`.
