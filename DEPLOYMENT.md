...
**notarytool's `--timeout` JSON goes to stderr, not stdout.** When `--timeout` fires, `notarytool --output-format json` emits its `{"id":..., "message":"Timeout..."}` final output on **stderr**. A naked `SUBMIT_JSON=$(xcrun notarytool ...)` captures only stdout → empty → jq returns null → log-fetch skipped even though we DO have a submission id. Always combine with `2>&1` at the end of the `xcrun` invocation so the capture sees both streams. Belt-and-suspenders: a UUID-regex fallback on the raw capture (`grep -oE '[0-9a-f]{8}-[0-9a-f]{4}-...'`) before giving up, in case notarytool ever mixes in non-JSON progress lines that confuse jq.

**`--no-s3-acceleration` as a hang workaround.** Apple's notary upload pipeline has two legs: the S3 upload and the backend processing. Community reports (Tauri, Electron) consistently point at the S3 leg as the more frequent stall point. Passing `--no-s3-acceleration` routes around the accelerated endpoint — slower in the happy case but much less likely to hang. Worth having on every submit call for CI reliability, not just during a debugging push.

**DMG-specific re-sign must walk every Mach-O inside-out.** A `find -name "*.framework"` sweep that signs only framework *bundles* is insufficient — notarytool checks every Mach-O the bundle contains, and it will flag: (a) loose `.dylib` / `.so` files some Flutter plugins drop under `Contents/Frameworks` or `Contents/Resources` without wrapping in a `.framework`, (b) the inner binary at `Versions/A/<FrameworkName>` which 2025-era Flutter engines sometimes need signed explicitly before the framework container. The correct pattern (Apple TN2206): `find -depth -type f | file --mime-type` to enumerate every Mach-O deepest-first, sign each with `--options runtime --timestamp`, then sign each `.framework` bundle, then the outer `.app` with entitlements. Never use `--deep` — explicitly discouraged by Apple and masks exactly these kinds of nested-code gaps.
...
- **Installer** — `pensine-v<version>-build<N>-setup.exe`. Per-user install (no admin), Start Menu entry, `.pensine` file association, proper uninstaller. Recommended path for most users.
- **Portable zip** — `pensine-v<version>-build<N>-windows.zip`. Extract anywhere, run `pensine.exe`. No file association, no uninstaller. Use case: USB-stick installs, locked-down corporate PCs that block installers but allow zip extraction, or anyone who wants to leave no trace on the host machine.
...
- Google Play closed-testing requirement for personal accounts created after Nov 2023: 12+ testers running for 14+ days before first production release. Internal track is unaffected. Does not apply to organization accounts. `local/copy_tester_email.bat` (runs `local/copy_tester_email.ps1`) puts the recruitment email on the Windows clipboard as rich HTML + plain-text fallback — paste into Gmail compose, send. Links go to the `pensine-testers` Google Group → Play Console tester-opt-in URL → Play Store listing, in that order (all three must use the same Google account for Play to match the tester).
